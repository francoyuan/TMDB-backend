...
import edu.whu.tmdb.query.utils.MapToDataType;
...
public class SelectImpl implements edu.whu.tmdb.query.excecute.Select {
...
//      //Values 也是一种plainSelect，如果是values，由values方法处理
    ...
    private SelectResult limit(int limit,SelectResult selectResult) {
        ...
        tpl.tuplenum=Math.min(collect.size(),limit);
        return selectResult;
    }
    ...
    private SelectResult groupByElicit(PlainSelect plainSelect, HashMap resultMap, SelectResult selectResult) throws TMDBException {
        ...
        result.setType(new DataType[length]);
    ...
    public SelectResult elicit(PlainSelect plainSelect,SelectResult selectResult) throws TMDBException {
        ...
        while(i<length){
            ...
            if(selectItemList.get(index).getClass().getSimpleName().equals("SelectExpressionItem")){
                ...
                result.getAlias()[i]=selectResult.getAttrname()[tempI];
                result.getType()[i]=selectResult.getType()[tempI];
                result.getAttrid()[i]=i;
                result.getClassName()[i]=selectResult.getClassName()[tempI];
                i++;
    ...
    public SelectResult values(ValuesStatement valuesStatement){
        ...
        for(int i=0;i<expressions.size();i++){
            if(expressions.get(i).getClass().getSimpleName().equals("RowConstructor")) {
                ...
                for (int j = 0; j < expressionList1.getExpressions().size(); j++) {
                    dataTypes.add(DataType.jspToDataType(expressionList1.getExpressions().get(j).getClass().getSimpleName()).get());
                    tuple[j] = MapToDataType.mapToDataType(expressionList1.getExpressions().get(j)).get();
                }
        ...
        for(int i=0;!tupleList.tuplelist.isEmpty()&&i<tupleList.tuplelist.get(0).tuple.length;i++){
            ...
            ClassTableItem classTableItem=new ClassTableItem("", -1, tupleList.tuplelist.get(0).tuple.length,i, "attr"+i,
                    dataTypes.get(i),
//                    DataType.mapToDataType(tupleList.tuplelist.get(0).tuple.getClass().getSimpleName()).get(),
                    "","");
    ...
    public SelectResult getSelectResult(ArrayList<ClassTableItem> classTableItemList,TupleList tupleList){
        ...
        selectResult.setType(new DataType[classTableItemList.size()]);
    ...
    public void getSelectColumn(SimpleNode node,ArrayList<Column> selectColumnList){
        int i=0;
        while(node!=null && i<node.jjtGetNumChildren()){
